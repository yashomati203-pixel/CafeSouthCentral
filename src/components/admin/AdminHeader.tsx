...
export default function AdminHeader({ title, onToggleSidebar, notificationsCount, actions }: AdminHeaderProps) {
    ...
    return (
        <header className="sticky top-0 z-30 h-16 md:h-20 bg-[#e8f5e9]/80 backdrop-blur-md border-b border-[#102214]/10 px-4 md:px-8 flex items-center justify-between">
            {/* Left: Mobile Toggle & Title */}
            <div className="flex items-center gap-4">
                <button
                    onClick={onToggleSidebar}
                    className="md:hidden p-2 rounded-lg text-[#102214]/50 hover:text-[#102214] focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500"
                >
                    <Menu className="h-6 w-6" aria-hidden="true" />
                </button>
                <div className="flex flex-col">
                    <h1 className="text-xl md:text-2xl font-serif font-black text-[#102214] tracking-tight">{title}</h1>
                    <p className="text-[10px] md:text-xs text-[#102214]/50 font-bold uppercase tracking-widest hidden sm:block">
            ...
            <div className="flex items-center gap-3 md:gap-6">
                ...
                {/* Search (Desktop) */}
                <div className="hidden lg:flex items-center bg-white/50 border border-[#102214]/10 rounded-full px-4 py-2 w-64 shadow-sm focus-within:ring-2 focus-within:ring-[#14b84b]/50 transition-all">
                    <Search className="w-4 h-4 text-[#102214]/50" />
                    <input
                        type="text"
                        placeholder="Search orders, items..."
                        className="bg-transparent border-none outline-none text-sm ml-2 w-full text-[#102214] placeholder-[#102214]/50"
                    />
                </div>


                {/* Lunch Bell & Alerts */}
                <div className="flex items-center gap-2 border-l border-[#102214]/10 pl-4 md:pl-6 ml-2 md:ml-0">
                    <button
                        onClick={() => {
                        ...
                        className="p-2 bg-[#102214]/5 text-[#102214] rounded-lg hover:bg-[#102214]/10 transition-colors group relative"
                    ...
                    <div className="relative group">
                        <button className="p-2 text-[#102214]/50 hover:text-[#102214] rounded-lg transition-colors">
                        ...
                        {/* Dropdown Menu */}
                        <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-[#102214]/10 p-2 invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all transform origin-top-right z-50">
                            <div className="text-xs font-bold text-[#102214]/50 uppercase px-3 py-2">Alert Settings</div>

                            {/* Campus Event Toggle */}
                            {/* High Demand Toggle */}
                            <button
                                onClick={() => toggleAlert('HIGH_DEMAND', 'Due to high demand, orders may be delayed by 15-20 mins.')}
                                className={`w-full text-left px-3 py-2 text-sm rounded-lg flex items-center justify-between group transition-colors ${activeAlert?.type === 'HIGH_DEMAND' ? 'bg-orange-500/20 text-orange-400' : 'text-[#102214]/70 hover:bg-[#102214]/5'}`}
                            >
                            ...
                            <button
                                ...
                                className={`w-full text-left px-3 py-2 text-sm rounded-lg flex items-center justify-between group transition-colors ${activeAlert?.type === 'WEATHER' ? 'bg-blue-500/20 text-blue-400' : 'text-[#102214]/70 hover:bg-[#102214]/5'}`}
                            >
                                <span className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className={`w-full text-left px-3 py-2 text-sm rounded-lg flex items-center justify-between group transition-colors ${activeAlert?.type === 'EVENT' ? 'bg-purple-500/20 text-purple-400' : 'text-[#102214]/70 hover:bg-[#102214]/5'}`}
                            >
                                <span className="flex items-center gap-2">
                            ...
                            <hr className="my-1 border-[#102214]/10" />

                            <button className="w-full text-left px-3 py-2 text-sm text-[#102214]/70 hover:bg-[#102214]/5 rounded-lg flex items-center gap-2">
                            ...
                            <button
                                ...
                                className="w-full text-left px-3 py-2 text-sm text-red-500 hover:bg-red-500/10 rounded-lg flex items-center justify-between group"
                            >
                                <span className="flex items-center gap-2">
                ...
                <div className="hidden md:flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-[#14b84b] text-white flex items-center justify-center font-bold text-xs shadow-lg">
                        A
                    </div>
                </div>
